...
class ProposalGenerator:
    ...
    def load_ideas_from_template(self, template_name: str) -> List[Dict[str, Any]]:
        ...
        # Pattern matches both old format (*_ideas.json) and new format (*_ideas_YYYYMMDD_HHMMSS.json)
    ...
    def generate_proposal(self, idea: Dict[str, Any], template_name: str, model_name: str = None) -> Dict[str, Any]:
        ...
        if template_name in ['generate_ideas_no_role', 'generate_diverse_ideas']:
            # No role - use unified template with None
            # Both no_role and diverse_ideas use the same approach (no role)
            role_to_use = None
        elif template_name in available_roles:
            # Use the unified template with the specified role
            role_to_use = template_name
        else:
            raise ValueError(f"Invalid template '{template_name}'. Available roles: {available_roles}, or use 'generate_ideas_no_role' or 'generate_diverse_ideas'")
...
def main():
    ...
    parser.add_argument('--template', '-t', 
                       ...
                       help='Template name to process (default: generate_ideas_no_role). Options: single_scientist, groups_of_scientists, groups_of_interdisciplinary_scientists, generate_ideas_no_role, generate_diverse_ideas')
    parser.add_argument('--model', '-m',
                       help='Filter to only process ideas generated by this model (e.g., grok-4, gpt-4, gemini-2.5-pro). This model will also be used to generate the proposals. If not specified, processes ideas from all models.')
